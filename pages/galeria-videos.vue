<template>
  <Head><Title>Videos - Concejo Municipal de Sucre</Title></Head>
  <main id="main">
    <!-- ======= Breadcrumbs ======= -->
    <div class="breadcrumbs">
      <div
        class="page-header d-flex align-items-center"
        style="background-image: url('/img/page-header.jpg')"
      >
        <div class="container position-relative">
          <div class="row d-flex justify-content-center">
            <div class="col-lg-6 text-center">
              <h2>Galeria - Videos</h2>
            </div>
          </div>
        </div>
      </div>
      <nav>
        <div class="container">
          <ol>
            <li><NuxtLink to="/">Inicio</NuxtLink></li>
            <li>Galeria - Videos</li>
          </ol>
        </div>
      </nav>
    </div>
    <!-- End Breadcrumbs -->
    <section id="about" class="about bg-white">
      <div class="container" data-aos="fade-up">
        <div class="section-header">
          <span>Galeria - Videos</span>
          <h2>Todas los <b>Videos</b></h2>
        </div>

        <div class="row">
          <div class="col-md-6 col-lg-4 col-xl-3 mb-3">
            <div
              class="swiper-slide slide-item swiper-slide-visible swiper-slide-active"
            >
              <div class="block-images position-relative">
                <div class="img-box">
                  <img
                    src="/img/strams/stream1.jpg"
                    class="img-fluid"
                    alt=""
                    loading="lazy"
                  />
                </div>
                <div class="block-description">
                  <div class="hover-buttons">
                    <a href="#" class="glightbox play-btn"></a>
                  </div>
                  <div class="detail-description">
                    <h6 class="iq-title">
                      <a href="video-detail.html">Titulo del Video</a>
                    </h6>

                    <div class="movie-time d-flex align-items-center">
                      <span class="text-white">Descripción del Videos</span>
                    </div>
                  </div>
                </div>
                <div class="block-social-info d-none">
                  <ul class="list-inline p-0 m-0 music-play-lists">
                    <li class="share">
                      <span><i class="fas fa-share"></i></span>
                      <div class="share-box">
                        <div
                          class="d-flex justify-content-center align-items-center gap-3"
                        >
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-facebook-f"></i
                          ></a>
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-twitter"></i
                          ></a>
                          <a
                            href="#"
                            data-link="#"
                            class="share-ico iq-copy-link"
                            tabindex="0"
                            ><i class="fas fa-link"></i
                          ></a>
                        </div>
                      </div>
                    </li>
                    <li>
                      <span><i class="fas fa-heart"></i></span>
                    </li>
                    <li>
                      <span><i class="fas fa-plus"></i></span>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 col-xl-3 mb-3">
            <div
              class="swiper-slide slide-item swiper-slide-visible swiper-slide-active"
            >
              <div class="block-images position-relative">
                <div class="img-box">
                  <img
                    src="/img/strams/stream1.jpg"
                    class="img-fluid"
                    alt=""
                    loading="lazy"
                  />
                </div>
                <div class="block-description">
                  <div class="hover-buttons">
                    <a href="#" class="glightbox play-btn"></a>
                  </div>
                  <div class="detail-description">
                    <h6 class="iq-title">
                      <a href="video-detail.html">Titulo del Video</a>
                    </h6>

                    <div class="movie-time d-flex align-items-center">
                      <span class="text-white">Descripción del Videos</span>
                    </div>
                  </div>
                </div>
                <div class="block-social-info d-none">
                  <ul class="list-inline p-0 m-0 music-play-lists">
                    <li class="share">
                      <span><i class="fas fa-share"></i></span>
                      <div class="share-box">
                        <div
                          class="d-flex justify-content-center align-items-center gap-3"
                        >
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-facebook-f"></i
                          ></a>
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-twitter"></i
                          ></a>
                          <a
                            href="#"
                            data-link="#"
                            class="share-ico iq-copy-link"
                            tabindex="0"
                            ><i class="fas fa-link"></i
                          ></a>
                        </div>
                      </div>
                    </li>
                    <li>
                      <span><i class="fas fa-heart"></i></span>
                    </li>
                    <li>
                      <span><i class="fas fa-plus"></i></span>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 col-xl-3 mb-3">
            <div
              class="swiper-slide slide-item swiper-slide-visible swiper-slide-active"
            >
              <div class="block-images position-relative">
                <div class="img-box">
                  <img
                    src="/img/strams/stream1.jpg"
                    class="img-fluid"
                    alt=""
                    loading="lazy"
                  />
                </div>
                <div class="block-description">
                  <div class="hover-buttons">
                    <a href="#" class="glightbox play-btn"></a>
                  </div>
                  <div class="detail-description">
                    <h6 class="iq-title">
                      <a href="video-detail.html">Titulo del Video</a>
                    </h6>

                    <div class="movie-time d-flex align-items-center">
                      <span class="text-white">Descripción del Videos</span>
                    </div>
                  </div>
                </div>
                <div class="block-social-info d-none">
                  <ul class="list-inline p-0 m-0 music-play-lists">
                    <li class="share">
                      <span><i class="fas fa-share"></i></span>
                      <div class="share-box">
                        <div
                          class="d-flex justify-content-center align-items-center gap-3"
                        >
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-facebook-f"></i
                          ></a>
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-twitter"></i
                          ></a>
                          <a
                            href="#"
                            data-link="#"
                            class="share-ico iq-copy-link"
                            tabindex="0"
                            ><i class="fas fa-link"></i
                          ></a>
                        </div>
                      </div>
                    </li>
                    <li>
                      <span><i class="fas fa-heart"></i></span>
                    </li>
                    <li>
                      <span><i class="fas fa-plus"></i></span>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 col-xl-3 mb-3">
            <div
              class="swiper-slide slide-item swiper-slide-visible swiper-slide-active"
            >
              <div class="block-images position-relative">
                <div class="img-box">
                  <img
                    src="/img/strams/stream1.jpg"
                    class="img-fluid"
                    alt=""
                    loading="lazy"
                  />
                </div>
                <div class="block-description">
                  <div class="hover-buttons">
                    <a href="#" class="glightbox play-btn"></a>
                  </div>
                  <div class="detail-description">
                    <h6 class="iq-title">
                      <a href="video-detail.html">Titulo del Video</a>
                    </h6>

                    <div class="movie-time d-flex align-items-center">
                      <span class="text-white">Descripción del Videos</span>
                    </div>
                  </div>
                </div>
                <div class="block-social-info d-none">
                  <ul class="list-inline p-0 m-0 music-play-lists">
                    <li class="share">
                      <span><i class="fas fa-share"></i></span>
                      <div class="share-box">
                        <div
                          class="d-flex justify-content-center align-items-center gap-3"
                        >
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-facebook-f"></i
                          ></a>
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-twitter"></i
                          ></a>
                          <a
                            href="#"
                            data-link="#"
                            class="share-ico iq-copy-link"
                            tabindex="0"
                            ><i class="fas fa-link"></i
                          ></a>
                        </div>
                      </div>
                    </li>
                    <li>
                      <span><i class="fas fa-heart"></i></span>
                    </li>
                    <li>
                      <span><i class="fas fa-plus"></i></span>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 col-xl-3 mb-3">
            <div
              class="swiper-slide slide-item swiper-slide-visible swiper-slide-active"
            >
              <div class="block-images position-relative">
                <div class="img-box">
                  <img
                    src="/img/strams/stream1.jpg"
                    class="img-fluid"
                    alt=""
                    loading="lazy"
                  />
                </div>
                <div class="block-description">
                  <div class="hover-buttons">
                    <a href="#" class="glightbox play-btn"></a>
                  </div>
                  <div class="detail-description">
                    <h6 class="iq-title">
                      <a href="video-detail.html">Titulo del Video</a>
                    </h6>

                    <div class="movie-time d-flex align-items-center">
                      <span class="text-white">Descripción del Videos</span>
                    </div>
                  </div>
                </div>
                <div class="block-social-info d-none">
                  <ul class="list-inline p-0 m-0 music-play-lists">
                    <li class="share">
                      <span><i class="fas fa-share"></i></span>
                      <div class="share-box">
                        <div
                          class="d-flex justify-content-center align-items-center gap-3"
                        >
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-facebook-f"></i
                          ></a>
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-twitter"></i
                          ></a>
                          <a
                            href="#"
                            data-link="#"
                            class="share-ico iq-copy-link"
                            tabindex="0"
                            ><i class="fas fa-link"></i
                          ></a>
                        </div>
                      </div>
                    </li>
                    <li>
                      <span><i class="fas fa-heart"></i></span>
                    </li>
                    <li>
                      <span><i class="fas fa-plus"></i></span>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 col-xl-3 mb-3">
            <div
              class="swiper-slide slide-item swiper-slide-visible swiper-slide-active"
            >
              <div class="block-images position-relative">
                <div class="img-box">
                  <img
                    src="/img/strams/stream1.jpg"
                    class="img-fluid"
                    alt=""
                    loading="lazy"
                  />
                </div>
                <div class="block-description">
                  <div class="hover-buttons">
                    <a href="#" class="glightbox play-btn"></a>
                  </div>
                  <div class="detail-description">
                    <h6 class="iq-title">
                      <a href="video-detail.html">Titulo del Video</a>
                    </h6>

                    <div class="movie-time d-flex align-items-center">
                      <span class="text-white">Descripción del Videos</span>
                    </div>
                  </div>
                </div>
                <div class="block-social-info d-none">
                  <ul class="list-inline p-0 m-0 music-play-lists">
                    <li class="share">
                      <span><i class="fas fa-share"></i></span>
                      <div class="share-box">
                        <div
                          class="d-flex justify-content-center align-items-center gap-3"
                        >
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-facebook-f"></i
                          ></a>
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-twitter"></i
                          ></a>
                          <a
                            href="#"
                            data-link="#"
                            class="share-ico iq-copy-link"
                            tabindex="0"
                            ><i class="fas fa-link"></i
                          ></a>
                        </div>
                      </div>
                    </li>
                    <li>
                      <span><i class="fas fa-heart"></i></span>
                    </li>
                    <li>
                      <span><i class="fas fa-plus"></i></span>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 col-xl-3 mb-3">
            <div
              class="swiper-slide slide-item swiper-slide-visible swiper-slide-active"
            >
              <div class="block-images position-relative">
                <div class="img-box">
                  <img
                    src="/img/strams/stream1.jpg"
                    class="img-fluid"
                    alt=""
                    loading="lazy"
                  />
                </div>
                <div class="block-description">
                  <div class="hover-buttons">
                    <a href="#" class="glightbox play-btn"></a>
                  </div>
                  <div class="detail-description">
                    <h6 class="iq-title">
                      <a href="video-detail.html">Titulo del Video</a>
                    </h6>

                    <div class="movie-time d-flex align-items-center">
                      <span class="text-white">Descripción del Videos</span>
                    </div>
                  </div>
                </div>
                <div class="block-social-info d-none">
                  <ul class="list-inline p-0 m-0 music-play-lists">
                    <li class="share">
                      <span><i class="fas fa-share"></i></span>
                      <div class="share-box">
                        <div
                          class="d-flex justify-content-center align-items-center gap-3"
                        >
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-facebook-f"></i
                          ></a>
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-twitter"></i
                          ></a>
                          <a
                            href="#"
                            data-link="#"
                            class="share-ico iq-copy-link"
                            tabindex="0"
                            ><i class="fas fa-link"></i
                          ></a>
                        </div>
                      </div>
                    </li>
                    <li>
                      <span><i class="fas fa-heart"></i></span>
                    </li>
                    <li>
                      <span><i class="fas fa-plus"></i></span>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-6 col-lg-4 col-xl-3 mb-3">
            <div
              class="swiper-slide slide-item swiper-slide-visible swiper-slide-active"
            >
              <div class="block-images position-relative">
                <div class="img-box">
                  <img
                    src="/img/strams/stream1.jpg"
                    class="img-fluid"
                    alt=""
                    loading="lazy"
                  />
                </div>
                <div class="block-description">
                  <div class="hover-buttons">
                    <a href="#" class="glightbox play-btn"></a>
                  </div>
                  <div class="detail-description">
                    <h6 class="iq-title">
                      <a href="video-detail.html">Titulo del Video</a>
                    </h6>

                    <div class="movie-time d-flex align-items-center">
                      <span class="text-white">Descripción del Videos</span>
                    </div>
                  </div>
                </div>
                <div class="block-social-info d-none">
                  <ul class="list-inline p-0 m-0 music-play-lists">
                    <li class="share">
                      <span><i class="fas fa-share"></i></span>
                      <div class="share-box">
                        <div
                          class="d-flex justify-content-center align-items-center gap-3"
                        >
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-facebook-f"></i
                          ></a>
                          <a
                            href="#"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="share-ico"
                            tabindex="0"
                            ><i class="fab fa-twitter"></i
                          ></a>
                          <a
                            href="#"
                            data-link="#"
                            class="share-ico iq-copy-link"
                            tabindex="0"
                            ><i class="fas fa-link"></i
                          ></a>
                        </div>
                      </div>
                    </li>
                    <li>
                      <span><i class="fas fa-heart"></i></span>
                    </li>
                    <li>
                      <span><i class="fas fa-plus"></i></span>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <!-- End About Us Section -->
  </main>
</template>
<script>
export default {
  beforeRouteLeave(to, from, next) {
    window.scrollTo(0, 0); // Vuelve al inicio de la página antes de salir de la página de inicio
    next();
  },
};
</script>
